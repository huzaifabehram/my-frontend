    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/85 backdrop-blur-sm p-2 sm:p-4" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-gray-900 rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          ...
          <button onClick={onClose} className="ml-3 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors flex-shrink-0">✕</button>
        </div>
        <div className="relative bg-black aspect-video">
          {ytId ? (
            ...
            <>
              ...
              {!playing && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/40 cursor-pointer" onClick={() => videoRef.current?.play()}>
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white/20 border-2 border-white/60 flex items-center justify-center hover:scale-110 transition-transform">
                    <svg className="w-6 h-6 sm:w-7 sm:h-7 text-white ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                  </div>
                </div>
              )}
            ...
            <div className="w-full h-full flex items-center justify-center"><div className="text-center"><div className="text-4xl mb-3">🎬</div><p className="text-white/60 text-sm">No preview available</p></div></div>
          )}
        </div>
        <div className="flex items-center justify-between px-4 py-3 bg-gray-800">
          ...
          <div className="flex gap-1.5 overflow-x-auto max-w-[180px] sm:max-w-xs">
            {lectures.map((_, i) => (
        ...
        <div className="max-h-36 overflow-y-auto divide-y divide-white/5">
          {lectures.map((lec, i) => (
            <button key={lec._id || i} onClick={() => setCurrentIdx(i)} className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${i === currentIdx ? "bg-amber-400/10" : "hover:bg-white/5"}`}>
    ...
    <div className="relative rounded-xl overflow-hidden shadow-2xl cursor-pointer group" onClick={onPreview}>
      ...
      <div className="absolute inset-0 bg-black/40 group-hover:bg-black/55 transition-colors flex items-center justify-center">
        <div className="text-center">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white/20 border-2 border-white/70 flex items-center justify-center mx-auto mb-2 sm:mb-3 group-hover:scale-110 transition-transform shadow-xl">
            <svg className="w-6 h-6 sm:w-7 sm:h-7 text-white ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
          </div>
          <p className="text-white font-bold text-sm sm:text-base">Preview this course</p>
...
// ─── NAVBAR ───────────────────────────────────────────────────────────────────
// Mobile: Logo + search icon (tappable) + compact auth buttons
// Desktop: Logo + search bar + full auth buttons
function Navbar({ user }) {
  const navigate = useNavigate();
  return (
    <nav className="bg-gray-900 text-white sticky top-0 z-50 shadow-lg">
      <div className="px-4 sm:px-6 flex items-center justify-between" style={{ minHeight: 56 }}>
        {/* Logo */}
        <button onClick={() => navigate("/courses")} className="text-lg sm:text-xl font-black text-amber-400 hover:text-amber-300 transition-colors py-3 flex-shrink-0">
          LearnFlow
        </button>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          {/* Mobile search icon */}
          <button className="md:hidden p-2 rounded-lg text-white/70 hover:text-white hover:bg-white/10 transition-colors" onClick={() => navigate("/courses")}>
          ...
          {user ? (
            <>
              ...
              <button onClick={() => navigate(user.role === "instructor" ? "/instructor" : "/portal")}
                className="text-xs sm:text-sm bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg transition-colors whitespace-nowrap">
                {user.role === "instructor" ? "Dashboard" : "My Learning"}
              </button>
            </>
          ) : (
            <>
              <Link to="/auth/login" className="text-xs sm:text-sm font-semibold text-slate-200 hover:text-white border border-slate-500 hover:border-slate-300 px-2.5 sm:px-4 py-1.5 rounded-lg transition-all whitespace-nowrap">
                Log In
              </Link>
              <Link to="/auth/register" className="text-xs sm:text-sm font-bold bg-amber-400 hover:bg-amber-300 text-gray-900 px-2.5 sm:px-4 py-1.5 rounded-lg transition-colors whitespace-nowrap">
                Sign Up
              </Link>
            </>
          )}
        ...
        {items.map((item, i) => (
          <span key={i} className="flex items-center gap-1.5">
            ...
            {item.href
              ...
              : <span className="text-slate-300 font-medium truncate max-w-[120px] sm:max-w-none">{item.label}</span>}
          </span>
        ))}
      </div>
    </div>
...
function ReviewsSection({ courseId, course, user, enrolled }) {
  ...
  return (
    <section className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 shadow-sm">
      <h2 className="text-xl sm:text-2xl font-extrabold text-gray-900 mb-4 sm:mb-5">Student Reviews</h2>

      <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 mb-5 pb-5 border-b border-gray-100">
        <div className="flex flex-row sm:flex-col items-center gap-4 sm:gap-0 sm:justify-center bg-amber-50 rounded-2xl p-4 sm:p-5 sm:min-w-[110px]">
          <span className="text-5xl font-black text-amber-500 leading-none">{avgRating > 0 ? avgRating.toFixed(1) : "—"}</span>
          <div className="flex flex-col items-start sm:items-center gap-1">
            <StarRating rating={avgRating} size="lg"/>
            <span className="text-xs text-gray-400 mt-0 sm:mt-1">Course Rating</span>
          </div>
        </div>
      ...
      {user && enrolled && !submitted && (
        <div className="mb-5 bg-indigo-50 rounded-xl p-4 sm:p-5">
          <h3 className="font-bold text-gray-800 mb-3">Leave a Review</h3>
          <div className="flex items-center gap-1 mb-3">
            {[1,2,3,4,5].map(s => (
              <button key={s} onClick={() => setMyRating(s)}>
                <svg className={`w-7 h-7 sm:w-8 sm:h-8 transition-colors ${s <= myRating ? "text-amber-400" : "text-gray-300 hover:text-amber-300"}`} fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                </svg>
              </button>
            ))}
          ...
          <textarea value={myComment} onChange={e => setMyComment(e.target.value)} rows={3}
            ...
            className="w-full border border-gray-200 rounded-xl px-3 sm:px-4 py-2.5 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none mb-3"/>
          {formError && <p className="text-xs text-red-500 mb-2">{formError}</p>}
          <button onClick={handleSubmit} disabled={submitting}
            className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold px-6 py-2.5 rounded-xl disabled:opacity-50 transition-colors">
        ...
        <>
          <div className="space-y-4 sm:space-y-5">
            {displayed.map((review, i) => (
              <div key={review._id || i} className="flex gap-3 sm:gap-4 pb-4 border-b border-gray-50 last:border-0 last:pb-0">
          ...
          {reviews.length > 4 && (
            <button onClick={() => setShowAll(s => !s)} className="mt-4 sm:mt-5 w-full py-2.5 rounded-xl border border-gray-200 text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors">
...
function CoursesListPage({ user }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {categories.length > 1 && (
        <div className="bg-white border-b border-gray-100 px-4 sm:px-6 py-2.5 sm:py-3 sticky top-[56px] z-30 shadow-sm">
          <div className="max-w-7xl mx-auto flex gap-2 overflow-x-auto pb-1">
            {categories.map(cat => (
              <button key={cat} onClick={() => setCategory(cat)}
                className={`flex-shrink-0 text-xs font-bold px-3 sm:px-4 py-1.5 rounded-full transition-all ${category === cat ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        {loading ? (
          ...
          <div className="text-center py-16 sm:py-24">
            ...
            <p className="text-gray-500 text-sm px-4">{search || category !== "All" ? "Try a different search or category." : "No published courses yet."}</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Breadcrumb items={[
        ...
        { label: course.title?.length > 28 ? course.title.slice(0, 28) + "…" : course.title },
      ]}/>

      {/* ══════════════════════════════════════════════════════════════════════
          MOBILE HERO — shown only on mobile (lg:hidden)
          Layout: thumbnail → course info dark bar → price strip
         ══════════════════════════════════════════════════════════════════════ */}
      <div className="lg:hidden">
        ...
        <div className="bg-gray-900 text-white px-4 pt-4 pb-5">
          ...
          <h1 className="text-xl font-extrabold leading-snug mb-2">{course.title}</h1>
          <p className="text-slate-300 text-sm leading-relaxed mb-3 line-clamp-2">
            {course.subtitle || course.description?.slice(0, 100)}
          </p>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs mb-3">
            {course.rating > 0 && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-slate-400">({course.totalRatings || 0})</span>
              </div>
            )}
            {course.studentsEnrolled > 0 && <span className="text-slate-300">{course.studentsEnrolled.toLocaleString()} students</span>}
          </div>
          {course.instructor && (
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-amber-400 flex items-center justify-center font-bold text-gray-900 text-xs flex-shrink-0">{course.instructor.name?.[0] || "I"}</div>
              <span className="text-xs text-slate-300">By <span className="text-amber-400 font-semibold">{course.instructor.name}</span></span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white px-4 py-3 border-b border-gray-100 flex items-center gap-3">
          <span className="text-2xl font-black text-gray-900">${course.discountPrice ?? course.price ?? 0}</span>
          {course.discountPrice && course.price && (
            <><span className="text-sm text-gray-400 line-through">${course.price}</span><span className="text-xs font-bold text-red-500 bg-red-50 rounded px-2 py-0.5">{Math.round(100 - (course.discountPrice / course.price) * 100)}% off</span></>
          )}
          {(!course.price || course.price === 0) && <span className="text-sm font-bold text-emerald-600 bg-emerald-50 rounded px-2 py-0.5">FREE</span>}
          {course.discountPrice && <span className="text-xs text-red-500 font-semibold ml-auto">⚡ Limited time!</span>}
        </div>
      </div>

      {/* ══════════════════════════════════════════════════════════════════════
          DESKTOP HERO — dark banner, text left, card floats right
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          {/* Left column */}
          <div className="flex-1 min-w-0 space-y-4 sm:space-y-6 py-4 sm:py-6 lg:pr-4">

            {/* What you'll learn */}
            {course.whatYouLearn?.length > 0 && (
              <section className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 shadow-sm">
                <h2 className="text-lg sm:text-2xl font-extrabold text-gray-900 mb-3 sm:mb-4">What you'll learn</h2>
            ...
            {/* Description */}
            <section className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 shadow-sm">
              <h2 className="text-lg sm:text-2xl font-extrabold text-gray-900 mb-2 sm:mb-3">Description</h2>
              <div className={`overflow-hidden transition-all duration-500 relative ${expanded ? "max-h-[600px]" : "max-h-24 sm:max-h-32"}`}>
                <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{course.description}</p>
                {!expanded && <div className="absolute bottom-0 left-0 right-0 h-12 sm:h-16 bg-gradient-to-t from-white to-transparent"/>}
              </div>
            ...
            {course.requirements?.length > 0 && (
              <section className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 shadow-sm">
                <h2 className="text-lg sm:text-xl font-extrabold text-gray-900 mb-2 sm:mb-3">Requirements</h2>
            ...
            {course.sections?.length > 0 && (
              <section className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 shadow-sm">
                <h2 className="text-lg sm:text-2xl font-extrabold text-gray-900 mb-1">Course Content</h2>
                    ...
                    <div key={section._id}>
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0 flex-1 pr-2">
                          <svg className={`w-4 h-4 text-gray-500 flex-shrink-0 transition-transform ${openSections.includes(section._id) ? "rotate-90" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/>
                          </svg>
                          <span className="font-semibold text-sm text-gray-800 truncate">{section.title}</span>
                        </div>
                        <span className="text-xs text-gray-500 flex-shrink-0">{section.lectures?.length || 0} lectures</span>
                      </button>
                        ...
                        <div className="divide-y divide-gray-50">
                          {section.lectures?.map((lec, li) => {
                            ...
                            return (
                              <div
                                key={lec._id || li}
                                className={`flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 transition-colors ${isPreviewable ? "hover:bg-purple-50 active:bg-purple-100 cursor-pointer" : ""}`}
                              ...
                              >
                                <div className="flex items-center gap-2 min-w-0 flex-1">
                                  ...
                                  <span className={`text-sm truncate ${isPreviewable ? "text-gray-800" : "text-gray-700"}`}>{lec.title}</span>
                                  {isPreviewable && (
                                    <span className="flex-shrink-0 text-xs text-purple-600 border border-purple-200 bg-purple-50 rounded px-1.5 sm:px-2 py-0.5 font-semibold ml-1">Preview</span>
                                  )}
                                </div>
                                {lec.duration ? <span className="text-xs text-gray-400 flex-shrink-0 ml-2 tabular-nums font-mono">{lec.duration}</span> : null}
                              </div>
                            );
                          })}
                        </div>
            ...
            {course.instructor && (
              <section className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 shadow-sm">
                <h2 className="text-lg sm:text-2xl font-extrabold text-gray-900 mb-3 sm:mb-5">Your Instructor</h2>
                <div className="flex items-start gap-3 sm:gap-5">
                  <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-full bg-purple-600 flex items-center justify-center text-white font-black text-xl sm:text-2xl flex-shrink-0">
                    {course.instructor.name?.[0] || "I"}
                  </div>
                  <div>
                    <h3 className="text-base sm:text-xl font-bold text-purple-700">{course.instructor.name}</h3>
                    {course.instructor.title && <p className="text-sm text-gray-500 mt-0.5">{course.instructor.title}</p>}
                    {course.instructor.bio   && <p className="mt-2 sm:mt-3 text-sm text-gray-700 leading-relaxed">{course.instructor.bio}</p>}
                  </div>
                </div>
              </section>
            )}
          ...
          {/* ── DESKTOP ONLY: floating sticky card ────────────────────────── */}
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          MOBILE STICKY BOTTOM CTA — always visible while scrolling
          Full-width button, fixed at bottom, only on mobile (lg:hidden)
         ══════════════════════════════════════════════════════════════════════ */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-2xl">
        <div className="px-4 py-3 flex items-center gap-3">
          <div className="flex-shrink-0">
            <div className="text-lg font-black text-gray-900 leading-none">${course.discountPrice ?? course.price ?? 0}</div>
            {course.discountPrice && <div className="text-xs text-gray-400 line-through">${course.price}</div>}
          </div>
          <button
            ...
            : enrolling  ? "bg-indigo-400 text-white cursor-not-allowed"
            : "bg-indigo-600 hover:bg-indigo-700 text-white"
            }`}
          >
            {enrolled   ? "✓ Go to Course →"
           : enrolling  ? "Enrolling..."
           : !course.price || course.price === 0 ? "Start for Free →"
           : "Start Subscription →"}
          </button>
        </div>
      </div>
