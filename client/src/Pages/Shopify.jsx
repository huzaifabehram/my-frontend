...
// DESIGN: Bold Coral Dark — near-black (#1c1917), coral/orange (#fb923c) accents
//
// ─── FIXES IN THIS VERSION ────────────────────────────────────────────────────
// FIX 1: Instructor data — reads every possible field path the backend may return
//         (/users/:id response OR course-embedded fields), so the instructor block
//         always populates regardless of backend field naming convention.
// FIX 2: Reviews display — reads BOTH courseData.reviews_list AND courseData.reviews
//         so it works regardless of which field name the backend uses. Handles both
//         populated {user:{name,avatar}} objects and plain {author,text} objects.
// FIX 3: Review submission — works for both logged-in AND guest users. Logged-in
//         users submit via API; guests are shown a "log in to review" prompt.
//         Submitted reviews appear IMMEDIATELY via optimistic UI. On API success
//         the full course is re-fetched to sync server state.
// ─────────────────────────────────────────────────────────────────────────────

import React, { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import {
  ChevronDown, Play, Star, Users, Clock, BookOpen, Menu, X,
  Search, Check, Award, Smartphone, Film, Download, Globe, Shield,
  ChevronLeft, ChevronRight, MessageCircle, Share2, Bookmark, ThumbsUp,
} from 'lucide-react';
...
function getBunnyEmbedUrl(url) {
  ...
  const pm = url.match(/player\.mediadelivery\.net\/play\/(\d+)\/([a-zA-Z0-9-]+)/);
  if (pm) return `https://iframe.mediadelivery.net/embed/${pm[1]}/${pm[2]}?autoplay=false&loop=false&muted=false&preload=true`;
  const bm = url.match(/video\.bunnycdn\.com\/play\/(\d+)\/([a-zA-Z0-9-]+)/);
  if (bm) return `https://iframe.mediadelivery.net/embed/${bm[1]}/${bm[2]}?autoplay=false&loop=false&muted=false&preload=true`;
  const gm = url.match(/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i);
  const lm = url.match(/\/(\d+)\//);
  if (gm && lm) return `https://iframe.mediadelivery.net/embed/${lm[1]}/${gm[1]}?autoplay=false&loop=false&muted=false&preload=true`;
  return null;
}
...
function ReelsVideoCard({ videoUrl, onClick }) {
  ...
  return (
    <div ref={containerRef} onClick={onClick}
      className="w-[150px] h-[266px] rounded-xl overflow-hidden flex-shrink-0 relative bg-stone-900 cursor-pointer group shadow-lg hover:shadow-2xl transition-all duration-300 border border-stone-700">
      {ytId ? (
        <>
          <img src={`https://img.youtube.com/vi/${ytId}/maxresdefault.jpg`} alt="Video thumbnail" className="w-full h-full object-cover" />
...
function AutoSlideImageTestimonials({ imageTestimonials }) {
  ...
  useEffect(() => {
    const onSW = () => pause();
    const onST = () => pause();
    const onWW = (e) => { if (isPausedRef.current && sectionRef.current && !sectionRef.current.contains(e.target)) resume(); };
    const onWT = (e) => { if (isPausedRef.current && sectionRef.current && !sectionRef.current.contains(e.target)) resume(); };
    const s = sectionRef.current;
    if (s) { s.addEventListener('wheel', onSW, { passive: true }); s.addEventListener('touchmove', onST, { passive: true }); }
    window.addEventListener('wheel', onWW, { passive: true });
    window.addEventListener('touchmove', onWT, { passive: true });
    return () => {
      if (s) { s.removeEventListener('wheel', onSW); s.removeEventListener('touchmove', onST); }
      window.removeEventListener('wheel', onWW);
      window.removeEventListener('touchmove', onWT);
    };
    ...
    <div ref={sectionRef} className="relative select-none">
      ...
      <div className="overflow-hidden" style={{
        ...
        <div className={`testimonial-track flex pb-3${isPaused ? ' paused' : ''}`} style={{ gap: '12px', width: 'max-content' }}>
          {doubled.map((t, idx) => (
            <div key={idx} className="flex-shrink-0 rounded-xl overflow-hidden relative shadow-lg border border-stone-700"
              style={{ width: '200px', height: '360px', cursor: 'default' }} aria-hidden={idx >= imageTestimonials.length}>
              <img src={t.imageUrl} alt={t.author || 'Student testimonial'} className="w-full h-full object-cover" draggable={false} />
              <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-black/75 to-transparent pointer-events-none" />
              {t.author && <p className="absolute bottom-3 left-3 right-3 text-white text-sm font-semibold pointer-events-none drop-shadow line-clamp-1">{t.author}</p>}
            </div>
          ))}
        </div>
      </div>
...
function VideoReelsView({ isOpen, onClose, videos, startIndex = 0 }) {
  ...
  useEffect(() => {
    videoRefs.current.forEach((v, idx) => {
      if (!v) return;
      if (idx === currentIndex) { v.muted = false; v.play().catch(() => {}); }
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 overflow-y-scroll snap-y snap-mandatory">
      <button onClick={onClose} className="fixed top-4 right-4 z-50 w-10 h-10 rounded-full bg-black/50 hover:bg-black/70 flex items-center justify-center text-white transition border-none cursor-pointer">
        <X size={24} />
      </button>
      {videos.map((video, index) => (
        <div key={index} className="h-screen w-full snap-start flex items-center justify-center relative">
          <VideoPlayer url={video.videoUrl} isReelsStyle autoPlay={index === currentIndex} muted={index !== currentIndex} loop controls={false} videoRef={(el) => (videoRefs.current[index] = el)} className="h-full w-full object-cover" />
...
function ImageSliderView({ isOpen, onClose, images, startIndex = 0 }) {
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex overflow-x-auto snap-x snap-mandatory">
      <button onClick={onClose} className="fixed top-4 right-4 z-50 w-10 h-10 rounded-full bg-black/50 hover:bg-black/70 flex items-center justify-center text-white transition border-none cursor-pointer"><X size={24} /></button>
      {images.map((image, index) => (
        <div key={index} className="min-w-full h-full snap-center flex items-center justify-center relative">
          ...
          <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-black/80 to-transparent pointer-events-none" />
          {index > 0 && <button onClick={() => setCurrentIndex(i => i - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition text-white z-10 border-none cursor-pointer"><ChevronLeft size={28} /></button>}
          {index < images.length - 1 && <button onClick={() => setCurrentIndex(i => i + 1)} className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition text-white z-10 border-none cursor-pointer"><ChevronRight size={28} /></button>}
...
// ── FIX 1 HELPER: Resolve instructor from ANY backend shape ──────────────────
// The /users/:id endpoint might return any combination of field names.
// We try every known variant so the UI always has data to display.
function resolveInstructor(instructorData, courseData) {
  if (instructorData) {
    return {
      name:     instructorData.name || instructorData.displayName || courseData.instructor || 'Instructor',
      rating:   instructorData.instructorRating  ?? instructorData.rating         ?? instructorData.averageRating ?? 0,
      reviews:  instructorData.instructorReviews ?? instructorData.totalReviews   ?? instructorData.reviewCount   ?? instructorData.reviews ?? 0,
      students: instructorData.instructorStudents ?? instructorData.totalStudents ?? instructorData.studentsEnrolled ?? instructorData.studentCount ?? 0,
      courses:  instructorData.instructorCourses  ?? instructorData.totalCourses  ?? instructorData.courseCount   ?? instructorData.courses ?? 0,
      bio:      instructorData.bio || instructorData.instructorBio || instructorData.about || instructorData.description || '',
      image:    instructorData.avatar || instructorData.instructorImage || instructorData.profileImage || instructorData.photo || '👩‍💼',
    };
  }
  // Fallback: use whatever fields exist directly on the course document
  return {
    name:     courseData.instructor       || courseData.instructorName     || 'Instructor',
    rating:   courseData.instructorRating ?? courseData.rating             ?? 0,
    reviews:  courseData.instructorReviews ?? courseData.reviewCount       ?? 0,
    students: courseData.instructorStudents ?? courseData.studentsEnrolled ?? 0,
    courses:  courseData.instructorCourses  ?? 0,
    bio:      courseData.instructorBio      || courseData.instructorAbout  || '',
    image:    courseData.instructorImage    || courseData.instructorAvatar || '👩‍💼',
  };
}

// ── FIX 2 HELPER: Normalise a review to a consistent shape ───────────────────
// Handles: { user:{_id,name,avatar}, text, rating, createdAt }  (populated)
//          { author:string, text, rating, date }                 (plain)
//          mixed combinations of the above
function normalizeReview(r, idx) {
  const authorName =
    r.author || r.userName || r.studentName ||
    r.user?.name || r.user?.displayName || 'Student';

  const avatarUrl =
    r.authorAvatar || r.avatar ||
    r.user?.avatar || r.user?.profileImage || null;

  const dateStr =
    r.date ||
    (r.createdAt ? new Date(r.createdAt).toLocaleDateString() : null) ||
    (r.created   ? new Date(r.created).toLocaleDateString()   : null) ||
    'Recently';

  return {
    _key:       r._id || r.id || `review-${idx}`,
    authorName,
    avatarUrl,
    rating:     r.rating ?? 5,
...
// ── Review avatar ─────────────────────────────────────────────────────────────
const AVATAR_COLORS = ['bg-orange-500', 'bg-stone-700', 'bg-amber-600', 'bg-red-600', 'bg-teal-600', 'bg-indigo-600'];
function ReviewAvatar({ authorName, avatarUrl, colorIndex }) {
  if (avatarUrl) return <img src={avatarUrl} alt={authorName} className="w-10 h-10 rounded-full object-cover flex-shrink-0" />;
  return (
    <div className={`w-10 h-10 rounded-full ${AVATAR_COLORS[colorIndex % AVATAR_COLORS.length]} text-white flex items-center justify-center font-bold text-sm flex-shrink-0`}>
...
export default function CourseLandingPage() {
  ...
  const { API: api, user } = useAuth();   // FIX 3: need user to check login state
  ...
  // Optimistic local reviews — appear immediately after submit
  ...
  // ── FIX 1: Fetch instructor — handle any instructorId field name ──────────
  useEffect(() => {
    if (!courseData) return;
    // Try every field name the course might store the instructor's user ID in
    const instructorId =
      courseData.instructorId      ||
      courseData.instructor_id     ||
      courseData.createdBy         ||
      courseData.author            ||
      // Only treat courseData.instructor as an ID if it looks like a MongoDB ObjectId
      (courseData.instructor && /^[a-f0-9]{24}$/.test(String(courseData.instructor))
        ? courseData.instructor : null);

    if (!instructorId) {
      setInstructorData(null);   // will use course-field fallback in resolveInstructor()
      return;
    }

    setLoadingInstructor(true);
    api.get(`/users/${instructorId}`)
      .then((res) => { setInstructorData(res.data); setLoadingInstructor(false); })
      .catch((err) => {
        console.warn('Could not load instructor from /users/:id — using course fields as fallback:', err.message);
  ...
  useEffect(() => {
    const onKey = (e) => { if (e.key === 'Escape' && isPreviewOpen) handleClosePreview(); };
    if (isPreviewOpen) { window.addEventListener('keydown', onKey); document.body.style.overflow = 'hidden'; }
    return () => { window.removeEventListener('keydown', onKey); document.body.style.overflow = 'unset'; };
  }, [isPreviewOpen]);

  // ── FIX 2: Merge reviews from BOTH possible backend field names ───────────
  const rawReviews = useMemo(() => {
    if (!courseData) return [];
    const a = Array.isArray(courseData.reviews_list) ? courseData.reviews_list : [];
    const b = Array.isArray(courseData.reviews)      ? courseData.reviews      : [];
    // Use whichever array is longer (backend may populate one but not both)
    return a.length >= b.length ? a : b;
  }, [courseData]);

  const allReviews = useMemo(() => {
    const serverNorm = rawReviews.map((r, i) => normalizeReview(r, i));
    // Keep local optimistic reviews that haven't been confirmed by server yet
    const localFiltered = localReviews.filter(
      (lr) => !serverNorm.some((sr) => sr.text === lr.text && sr.authorName === lr.authorName)
    );
    return [...localFiltered, ...serverNorm];
  }, [rawReviews, localReviews]);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    // Add optimistic review immediately so the user sees it right away
    const optimistic = {
      _key:          'optimistic-' + Date.now(),
      authorName:    user.name || user.displayName || user.email?.split('@')[0] || 'You',
    ...
    try {
      // api from useAuth already carries the Bearer token in its default headers
      ...
      // Re-fetch so the server-confirmed review replaces the optimistic one
      const updated = await fetchCourseById(courseData._id);
      if (updated) setFullCourse(updated);
    } catch (err) {
      console.error('Review submit error:', err);
      // Roll back the optimistic update
      setLocalReviews((prev) => prev.filter((r) => r._key !== optimistic._key));
  ...
  if (!courseData) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-stone-950 gap-4">
        ...
        <button onClick={() => navigate('/courses')} className="px-6 py-3 bg-orange-500 text-white font-semibold rounded-lg hover:bg-orange-400 transition border-none cursor-pointer">Browse all courses</button>
      </div>
    );
  }

  const instructor      = resolveInstructor(instructorData, courseData);
  ...
  const discountPct = courseData.originalPrice > courseData.price
    ? Math.round((1 - courseData.price / courseData.originalPrice) * 100) : null;
    ...
    <div className="min-h-screen bg-stone-950 overflow-x-hidden w-full">
      ...
      {isPreviewOpen && (
        <div className="fixed inset-0 z-[9999] bg-black/95 flex flex-col">
          <div className="absolute top-4 right-4 z-10">
            <button onClick={handleClosePreview} className="p-3 bg-white/10 hover:bg-white/20 rounded-full transition border-none cursor-pointer">
          ...
          <div className="w-full bg-stone-950 border-b border-stone-800 py-4 md:py-6">
            <div className="max-w-7xl mx-auto px-4 md:px-6">
              ...
              <h2 className="text-xl md:text-2xl font-bold text-white">{courseData.title}</h2>
            </div>
          </div>
          <div className="w-full bg-black"><div className="max-w-7xl mx-auto"><VideoPlayer url={currentVideo} className="w-full" /></div></div>
          <div className="flex-1 overflow-y-auto bg-stone-950">
            <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8">
              <h3 className="text-lg font-bold text-white mb-4">Free preview lectures</h3>
              {previewLectures.length === 0
                ? <p className="text-stone-400 text-base text-center py-12">No preview lectures available</p>
                : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {previewLectures.map((lecture) => (
                      <div key={`${lecture.sectionIdx}-${lecture.lectureIdx}`} onClick={() => setCurrentVideo(lecture.videoUrl)}
                        className="bg-stone-800/50 hover:bg-stone-800 border border-stone-700 rounded-lg p-3 cursor-pointer transition group">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-sm text-white group-hover:text-orange-400 transition truncate">{lecture.title}</p>
                            <p className="text-xs text-stone-400 mt-1">{lecture.sectionTitle}</p>
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      <header className="sticky top-0 z-40 bg-stone-950 border-b border-stone-800 w-full">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-3 md:py-4 flex items-center justify-between">
          <button className="lg:hidden p-2 text-stone-300 border-none cursor-pointer bg-transparent" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            {mobileMenuOpen ? <X size={22} /> : <Menu size={22} />}
          </button>
          <div className="absolute left-1/2 -translate-x-1/2 lg:relative lg:left-auto lg:transform-none">
            <button onClick={() => handleNavigate('/')} className="text-xl md:text-2xl font-bold text-orange-400 hover:text-orange-300 transition cursor-pointer bg-transparent border-none p-0">Courseify</button>
          </div>
          <nav className="hidden lg:flex items-center gap-8 flex-1 ml-12">
            {['Categories', 'Instructor', 'About'].map((label) => (
              <button key={label} onClick={() => handleNavigate(label === 'Categories' ? '/courses' : label === 'Instructor' ? '/instructor' : '/')}
                className="text-sm text-stone-400 hover:text-orange-400 transition bg-transparent border-none cursor-pointer p-0 font-medium">{label}</button>
            ))}
          </nav>
          <div className="flex items-center gap-2 md:gap-3">
            <Search className="hidden lg:block text-stone-500 cursor-pointer hover:text-stone-300 transition" size={20} />
            <button onClick={() => handleNavigate('/auth/login')} className="px-4 md:px-5 py-2 bg-orange-500 hover:bg-orange-400 text-white rounded-lg transition font-semibold border-none cursor-pointer text-sm shadow-sm">Log in</button>
          </div>
        </div>
        {mobileMenuOpen && (
          <>
            ...
            <div className="fixed top-0 left-0 h-full w-64 bg-stone-900 z-50 lg:hidden shadow-2xl border-r border-stone-700">
              <div className="p-6 space-y-2">
                ...
                {['Categories', 'Instructor', 'About'].map((label) => (
                  <button key={label} onClick={() => handleNavigate(label === 'Categories' ? '/courses' : label === 'Instructor' ? '/instructor' : '/')}
                    className="block w-full text-left text-stone-300 hover:text-orange-400 bg-transparent border-none cursor-pointer p-3 rounded-lg hover:bg-stone-800 font-medium transition text-sm">{label}</button>
                ))}
              </div>
            </div>
          </>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
            <div className="lg:col-span-2">
              <p className="text-xs font-semibold uppercase tracking-widest text-orange-400 mb-3">{courseData.category || 'Development'}</p>
              <div className="flex gap-2 flex-wrap mb-4">
                {courseData.bestseller && <span className="inline-flex items-center gap-1 bg-stone-800 border border-orange-500/40 text-orange-300 font-semibold px-3 py-1 rounded text-xs"><Award size={12} /> Bestseller</span>}
                {courseData.level && <span className="inline-block bg-stone-800 border border-stone-600 text-stone-300 font-medium px-3 py-1 rounded text-xs">{courseData.level}</span>}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 mb-4">
                {courseData.rating > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-0.5">{Array.from({ length: 5 }).map((_, i) => <Star key={i} size={14} className="text-orange-400" fill={i < Math.floor(courseData.rating) ? 'currentColor' : 'none'} />)}</div>
                    <span className="text-stone-500 text-xs">({courseData.reviews?.toLocaleString()} ratings)</span>
                  </div>
                )}
                {courseData.students > 0 && <div className="flex items-center gap-1.5 text-stone-400 text-sm"><Users size={14} /><span>{courseData.students.toLocaleString()} students</span></div>}
              </div>
              <div className="mb-4">
                <p className="text-stone-500 text-sm">Created by <button onClick={() => handleNavigate('/instructor')} className="text-orange-400 hover:text-orange-300 font-semibold bg-transparent border-none cursor-pointer p-0 underline">{instructor.name}</button></p>
              </div>
            ...
            <div className="hidden lg:block">
              <div className="sticky top-24">
                <div className="bg-stone-900 border border-stone-700 rounded-xl overflow-hidden">
                  <div className="relative aspect-video cursor-pointer group" onClick={handlePreviewClick}><CourseThumbnail course={courseData} /></div>
                  <div className="p-5">
                    <div className="flex items-baseline gap-3 mb-1"><span className="text-2xl font-bold text-white">{priceLabel}</span></div>
                    {discountPct && <div className="flex items-center gap-2 mb-4"><span className="text-sm text-stone-500 line-through">PKR {(courseData.originalPrice * 280).toLocaleString()}</span><span className="text-xs font-semibold text-orange-400">{discountPct}% off</span></div>}
                    <button onClick={() => handleNavigate('/auth/register')} className="w-full bg-orange-500 hover:bg-orange-400 text-white font-bold py-3 rounded-lg transition text-sm border-none cursor-pointer shadow-lg mb-2">Enroll now</button>
                    <button onClick={() => handleNavigate('/auth/register')} className="w-full bg-transparent hover:bg-stone-800 text-stone-300 font-semibold py-3 rounded-lg transition text-sm border border-stone-600 cursor-pointer mb-4">Buy now</button>
                  ...
                  <div className="border-t border-stone-800 px-5 py-4">
                    ...
                    {[
                      ...
                      <div key={idx} className="flex items-center gap-2.5 mb-2"><Icon size={15} className="text-stone-500 flex-shrink-0" /><p className="text-xs text-stone-400">{text}</p></div>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
            <div className="lg:col-span-2 w-full min-w-0">
                ...
                <>
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2 text-stone-500 text-xs mt-1">
                      ...
                      <span>•</span><span>{totalLectures} lectures</span>
                    ...
                    {sections.map((section, idx) => {
                      ...
                      return (
                        <div key={idx} className="border border-stone-200 rounded-xl overflow-hidden w-full bg-white">
                          <button onClick={() => setExpandedSection(isExpanded ? expandedSection.filter((i) => i !== idx) : [...expandedSection, idx])}
                            className="w-full px-4 py-3 flex items-center justify-between bg-stone-50 hover:bg-stone-100 transition border-none cursor-pointer">
                                ...
                                <div key={lectureIdx} className="px-4 py-2.5 border-b border-stone-100 last:border-b-0 flex items-center justify-between hover:bg-stone-50 transition">
                                  ...
                                  <div className="flex items-center gap-2 flex-shrink-0">
                                    ...
                                    {lecture.preview && lecture.videoUrl && (
                                      <button onClick={() => { setCurrentVideo(lecture.videoUrl); setIsPreviewOpen(true); }}
                                        className="flex items-center gap-1 text-orange-500 hover:text-orange-400 font-semibold text-xs cursor-pointer bg-transparent border-none whitespace-nowrap transition p-0">
                                        <div className="w-4 h-4 rounded-full bg-orange-500 flex items-center justify-center"><Play size={7} className="text-white ml-0.5" fill="currentColor" /></div>
                ...
                <div className="mb-10 w-full">
                  ...
                  <ul className="space-y-2">
                    {courseData.requirements.map((req, idx) => (
                      <li key={idx} className="flex gap-2 text-stone-600 text-sm"><span className="text-stone-400 flex-shrink-0 mt-0.5">•</span>{req}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {courseData.description && (
                <div className="mb-10 w-full">
                  ...
                  <div className="relative">
                    <div className={`text-stone-600 leading-relaxed text-sm prose prose-sm max-w-none ${!showFullDescription ? 'max-h-40 overflow-hidden' : ''}`}
                      style={{ wordBreak: 'break-word' }} dangerouslySetInnerHTML={{ __html: courseData.description }} />
              ...
              {/* ═══════════════════════════════════════════════════════════════
                  FIX 1 — INSTRUCTOR SECTION
                  Resolves data from every possible backend field path.
              ═══════════════════════════════════════════════════════════════ */}
                ...
                {loadingInstructor ? (
                  ...
                  <>
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      <div className="flex-shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-full bg-stone-800 border-2 border-orange-500/40 flex items-center justify-center shadow-lg overflow-hidden">
                    ...
                    {instructor.bio ? (
                      <div className="relative">
                        <div className={`text-stone-600 leading-relaxed text-sm ${!showFullInstructorBio ? 'max-h-16 overflow-hidden' : ''}`}><p>{instructor.bio}</p></div>
                      ...
                      <p className="text-stone-400 text-sm italic">No bio available for this instructor.</p>
                    )}
                  </>
                )}
              </div>
              ...
              {/* ═══════════════════════════════════════════════════════════════
                  FIX 2 — REVIEWS DISPLAY
                  Reads from both courseData.reviews AND courseData.reviews_list.
                  Normalises every review regardless of field names used.
              ═══════════════════════════════════════════════════════════════ */}
              <div className="mb-10 pt-6 border-t border-stone-200 w-full">
                ...
                {allReviews.length > 0 ? (
                  <div className="flex gap-3 overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 snap-x snap-mandatory" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                    {allReviews.map((review, idx) => (
                      <div key={review._key} className="flex-shrink-0 w-[82vw] sm:w-[calc(50%-6px)] lg:w-[calc(33.333%-11px)] snap-start">
                        <div className={`bg-white rounded-xl border ${review._isOptimistic ? 'border-orange-300 ring-1 ring-orange-200' : 'border-stone-200'} p-4 h-full`}>
                          <div className="flex items-start gap-3 mb-3">
                            <ReviewAvatar authorName={review.authorName} avatarUrl={review.avatarUrl} colorIndex={idx} />
                            <div className="flex-1 min-w-0">
                              <p className="font-semibold text-stone-900 text-sm flex items-center gap-2 flex-wrap">
                                {review.authorName}
                                {review._isOptimistic && <span className="text-xs text-orange-500 font-normal italic">(posting…)</span>}
                              </p>
                              <div className="flex items-center gap-2 mt-0.5">
                                <div className="flex gap-0.5">{Array.from({ length: 5 }).map((_, i) => <Star key={i} size={11} className="text-orange-400" fill={i < review.rating ? 'currentColor' : 'none'} />)}</div>
                                <span className="text-xs text-stone-400">• {review.dateStr}</span>
                              </div>
                            </div>
                          </div>
              ...
              {/* ═══════════════════════════════════════════════════════════════
                  FIX 3 — WRITE A REVIEW
                  • Logged-out users see a "Log in to review" prompt
                  • Logged-in users see the star + textarea form
                  • Optimistic update shows the review immediately
                  • On success: re-fetch course; on failure: roll back + show error
              ═══════════════════════════════════════════════════════════════ */}
              <div className="mb-10 pt-6 border-t border-stone-200 w-full">
                ...
                {reviewSuccess && (
                  <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-xl flex items-center gap-2">
                    ...
                    <p className="text-green-700 text-sm font-medium">{reviewSuccess}</p>
                  </div>
                )}

                {/* Guest prompt */}
                {!user ? (
                  <div className="bg-stone-100 border border-stone-200 rounded-xl p-5 flex flex-col sm:flex-row items-center gap-4">
                    <div className="flex-1">
                      <p className="text-stone-700 font-semibold text-sm mb-1">Want to leave a review?</p>
                      <p className="text-stone-500 text-xs">Log in to share your experience with other students.</p>
                    </div>
                    <button onClick={() => handleNavigate('/auth/login')}
                      className="bg-orange-500 hover:bg-orange-400 text-white font-bold py-2.5 px-5 rounded-lg transition text-sm border-none cursor-pointer whitespace-nowrap shadow-md">
                      Log in to review
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    {!showReviewForm && !reviewSuccess && (
                      <button onClick={() => { setShowReviewForm(true); setReviewError(''); }}
                        className="w-full bg-orange-500 hover:bg-orange-400 text-white font-bold py-3 rounded-xl transition text-sm border-none cursor-pointer shadow-md">
                        Write a review
                      </button>
                    )}
                    ...
                    {/* Review form */}
                    {showReviewForm && (
                      <form onSubmit={handleReviewSubmit} className="bg-white rounded-xl p-4 border border-stone-200 shadow-sm">
                        <h3 className="text-sm font-semibold text-stone-900 mb-3">Your rating</h3>

                        {/* Error banner */}
                        {reviewError && (
                          <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                            <p className="text-red-600 text-xs font-medium">{reviewError}</p>
                          </div>
                        )}
                        ...
                        <textarea
                          ...
                          rows={4}
                          className="w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm text-stone-900 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none bg-stone-50"
                        />

                        {/* Buttons */}
                        <div className="flex gap-2 mt-3">
                          <button type="submit" disabled={submittingReview}
                            className="flex-1 bg-orange-500 hover:bg-orange-400 text-white font-bold py-3 rounded-lg transition border-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed text-sm">
                            {submittingReview ? (
                          ...
                          <button type="button" onClick={() => { setShowReviewForm(false); setReviewError(''); }}
                            className="px-4 py-3 bg-stone-100 hover:bg-stone-200 text-stone-600 font-semibold rounded-lg transition border-none cursor-pointer text-sm">
                            Cancel
                          </button>
                        </div>
                      </form>
                ...
                <div className="mb-10 pt-6 border-t border-stone-200 w-full">
                  ...
                  <div className="flex gap-3 overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0 pb-4">
                    {videoTestimonials.map((testimonial, idx) => (
                      <ReelsVideoCard key={idx} videoUrl={testimonial.videoUrl} onClick={() => { setVideoReelsStartIndex(idx); setVideoReelsOpen(true); }} />
                    ))}
                  </div>
                </div>
              )}
                ...
                <div className="bg-white border border-stone-200 rounded-xl p-5">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={idx} className="flex items-center gap-2.5"><Icon size={17} className="text-stone-400 flex-shrink-0" /><p className="text-sm text-stone-600">{text}</p></div>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
                {studentsBoughtCourses.map((course) => (
                  <div key={course._id} onClick={() => navigate(`/course/${course._id}`)}
                    className="bg-white border border-stone-200 rounded-xl overflow-hidden hover:border-orange-300 transition cursor-pointer group">
                    <div className="h-32 md:h-40 bg-stone-100 flex items-center justify-center relative overflow-hidden">
                      {course.thumbnail ? <img src={course.thumbnail} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" /> : <span className="text-4xl">{course.emoji || '📚'}</span>}
                    </div>
                    <div className="p-3">
      ...
      <footer className="bg-stone-950 text-stone-500 py-10 md:py-14 w-full border-t border-stone-800">
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-6 border-t border-stone-800">
            <button onClick={() => handleNavigate('/')} className="text-lg font-bold text-orange-400 hover:text-orange-300 transition cursor-pointer bg-transparent border-none p-0 mb-4 md:mb-0">Courseify</button>
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:hidden bg-stone-900 border-t border-stone-700 p-3 z-50 flex items-center justify-between gap-3 w-full shadow-2xl">
        <div className="flex flex-col min-w-0">
          <span className="text-lg font-bold text-white truncate">{priceLabel}</span>
          {discountPct && <span className="text-xs text-orange-400 font-semibold">{discountPct}% off</span>}
        </div>
        <button onClick={() => handleNavigate('/auth/register')} className="flex-1 bg-orange-500 hover:bg-orange-400 text-white font-bold py-2.5 rounded-xl transition text-sm border-none cursor-pointer shadow-lg whitespace-nowrap">Enroll now</button>
      </div>
