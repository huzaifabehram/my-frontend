...
// UPDATED: Facebook-style cards for Video Reviews & Image Testimonials
// Mobile responsive with proper card UI, modals, and social actions
import React, { useState, useEffect, useMemo } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { ChevronDown, Play, Star, Users, Clock, BookOpen, Menu, X, Search, Check, Award, Smartphone, Film, Download, Globe, Shield, ChevronLeft, ChevronRight, Heart, MessageCircle } from 'lucide-react';
...
function Modal({ isOpen, onClose, children }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black bg-opacity-90 flex items-center justify-center p-4 animate-fadeIn">
      <button
        onClick={onClose}
...
function SocialCard({ children, onLike, onComment, likes = 0, comments = 0, className = "" }) {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 ${className}`}>
      ...
      <div className="px-4 py-3 border-t border-gray-100">
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
          ...
          <button
            onClick={onComment}
            className="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg text-gray-600 hover:bg-gray-100 transition-all"
          >
...
function VideoCard({ video, onClick }) {
  ...
  return (
    <SocialCard likes={Math.floor(Math.random() * 50) + 10} comments={Math.floor(Math.random() * 20) + 3}>
      <div className="relative aspect-video bg-gray-900 cursor-pointer group" onClick={onClick}>
        {thumbnail ? (
          <img src={thumbnail} alt={video.author} className="w-full h-full object-cover" />
        ) : (
        ...
        <div className="absolute inset-0 bg-black bg-opacity-30 group-hover:bg-opacity-40 transition flex items-center justify-center">
          <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-full bg-white bg-opacity-90 group-hover:bg-opacity-100 group-hover:scale-110 transition-all flex items-center justify-center shadow-2xl">
            <Play size={32} className="text-purple-600 ml-1" fill="currentColor" />
          </div>
        </div>

        <div className="absolute bottom-3 left-3 bg-black bg-opacity-70 px-3 py-1 rounded-full">
          <span className="text-white text-xs font-semibold">🎬 Video Review</span>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <p className="font-bold text-gray-900">{video.author}</p>
            <p className="text-xs text-gray-500">Student</p>
          </div>
        </div>
        <p className="text-gray-700 text-sm leading-relaxed">{video.text}</p>
      </div>
    </SocialCard>
  );
}
...
function ImageCard({ testimonial, onClick }) {
  return (
    <SocialCard likes={Math.floor(Math.random() * 80) + 15} comments={Math.floor(Math.random() * 30) + 5}>
      <div
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center text-white font-bold flex-shrink-0">
...
export default function CourseLandingPage() {
  ...
  }, [isPreviewOpen, videoModalOpen, imageModalOpen]);
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-bold">
                  {selectedVideo.author.charAt(0)}
                </div>
                <div>
                  <p className="font-bold text-gray-900">{selectedVideo.author}</p>
                  <p className="text-sm text-gray-500">Student</p>
                </div>
              </div>
              <p className="text-gray-700 leading-relaxed">{selectedVideo.text}</p>
            </div>
          </div>
        )}
      </Modal>
          ...
          <div className="bg-white rounded-lg overflow-hidden">
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-bold text-gray-900">{selectedImage.author}</p>
                  <p className="text-sm text-gray-500">Student</p>
                </div>
              </div>
              <p className="text-gray-700 leading-relaxed">{selectedImage.text}</p>
            </div>
          </div>
        )}
      </Modal>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              ...
              {/* 🆕 IMAGE TESTIMONIALS - FACEBOOK CARDS */}
              {imageTestimonials.length > 0 && (
                <div className="mb-12 pt-8 border-t border-gray-200">
                  <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">Student Testimonials</h2>
                  <p className="text-sm text-gray-600 mb-6">Real feedback from our students</p>
                  
                  <div className="overflow-x-auto pb-4 -mx-4 px-4">
                    <div className="flex gap-4 lg:gap-6" style={{ minWidth: 'min-content' }}>
              ...
              {/* 🆕 VIDEO TESTIMONIALS - FACEBOOK CARDS */}
              {videoTestimonials.length > 0 && (
                <div className="mb-12 pt-8 border-t border-gray-200">
                  <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">Video Reviews</h2>
                  <p className="text-sm text-gray-600 mb-6">Watch what our students have to say</p>
